...
def iv_value(file,feature,sep,target):
    ...
    for index, item in enumerate(dict_bin):
        try:
            woe_bin[index] = math.log((float(len_dict_bin_1[index]) / float(len_data_1)+0.0001) /
                                               (float(len_dict_bin_0[index]) / float(len_data_0)+0.0001))
            iv_bin[index] = ((float(len_dict_bin_1[index]) / float(len_data_1)+0.0001) 
                            - (float(len_dict_bin_0[index]) / float(len_data_0)+0.0001)) \
                            *woe_bin[index]
    ...
    df = pd.DataFrame(dict_result)  #通过字典构建Dataframe
    ...
    # 调整列的顺序
    #['class', 'bad','good', '%bad', '%good', 'all', 'woe', 'iv']
    df = df.loc[:, ['class', 'bad','good', '%bad', '%good', 'all','woe', 'iv']]
    df=df.iloc[:-2,:]
    return iv_sum,df
...
#方法调用
#'''====================================================='''
#a=iv_value('./KS_data.csv','loan_amnt',(500, 10450, 10475.0),'result')
#print(a[0])
#print(a[1])
